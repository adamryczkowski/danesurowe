...
compare_two_variables<-function(var1, var2)
{
  if (length(setdiff(class(var1), class(var2)))>0)  {
    ...
    if(classes_sorted == 'factor|labelled')  {
      ...
      if(class(var1)=='Date') {
        var1 <- as.numeric(var1 - as.Date(0, origin="1899-12-30", tz='UTC'))
      } else {
        var2 <- as.numeric(var2 - as.Date(0, origin="1899-12-30", tz='UTC'))
      }
...
convert_to_string<-function(var, row)
{
  ...
  } else if (vartype %in% c('I', 'N', 'D', 'S', 'F'))  {
...
#  if (srcrow==318) browser()
  ...
  if (!is.na(srcval))  {
    if(srcdesttype %in% c('ND', 'ID'))  {
      ...
      value <- as.numeric(srcval -as.Date(0, origin="1899-12-30", tz='UTC'))
      if (desttype == 'I') {
...
comment_diffs<-function(diffdb, df1, df1keys, df2, df2keys)
{
  db <- copy(diffdb)
  #TODO:
  out <- list()
  ...
  while(nrow(db)) {
    ...
    if(rownames[[1]]>colnames[[1]]) {
      ...
      {
        msg <- comment_one_diff(i, diffdb = tmpdb, df1 = df1, df1keys = df1keys, df2 = df2, df2keys = df2keys, flag_say_colname = TRUE)
        outvec[[i]] <- msg
      }
      ...
      for (i in seq(nrow(tmpdb)))
      {
        msg <- comment_one_diff(i, diffdb = tmpdb, df1 = df1, df1keys = df1keys, df2 = df2, df2keys = df2keys, flag_say_rowname =  TRUE)
        outvec[[i]] <- msg
      }
      outrec <- list(msg = paste0(
...
comment_one_diff<-function(rownr, diffdb, df1, df1keys, df2, df2keys, flag_say_colname = FALSE, flag_say_rowname = FALSE)
{
  ...
  if (flag_say_colname) {
    ...
    if (is.null(attr(df2[[colname]], 'label'))){
      varlabel <- colname
    } else {
      varlabel <- paste0(attr(df2[[colname]], 'label'), ' (', colname, ')')
    }
  ...
  if (flag_say_rowname) {
    rowlabel<-paste0(rowname, ' (that corresponds to source row nr ', match(rowname, df1keys), ' and destination row nr ', match(rowname, df2keys), ')')
